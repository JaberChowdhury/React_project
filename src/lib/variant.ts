import { HSLColor, ColorObject } from "@/types/variant";

const opositeColor = (hexadecimal: string): ColorObject | undefined => {
  const checkValidity = (): boolean => {
    const hexadecimal_code = "0123456789abcdef".toUpperCase().split("");
    let result = true;
  ...
  if (hexadecimal && isValid && hexadecimal.length === 6) {
    ...
    if (hexadecimal_colors) {
      ...
      console.log({ hexadecimal_colors });
      return undefined;
    }
  } else {
    console.log({ isValid, hexadecimal_length: hexadecimal.length });
    return undefined;
  }
};
...
console.log(
  JSON.parse(JSON.stringify(opositeColor(getHexadecimal()), null, 4)),
);
*/

export default opositeColor;
