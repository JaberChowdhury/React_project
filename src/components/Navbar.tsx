...
export default function Navbar() {
  return (
    <nav className="navbar fixed top-0 z-30 bg-base-100/50 backdrop-blur-md">
      <div className="navbar-start">
        <div className="dropdown">
          ...
          <ul
            tabIndex={0}
            className="menu menu-sm dropdown-content border border-cyan-400  mt-3 z-[1] p-2 shadow bg-base-100 rounded w-52"
          >
            {
              navigation.map(data=>{
                return <li key={data.title}><Link to={data.to}><Icon icon={data.icon || ""} />{data.title}</Link></li>
              })
            }
          </ul>
        </div>
        <Link to="/" className="btn btn-ghost text-xl">
          <span className="inline-grid text-3xl font-bold">
            <span
